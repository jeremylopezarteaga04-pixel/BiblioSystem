...
try {
    ...
    $consulta = $conexion->prepare('SELECT p.*, l.titulo FROM prestamos p INNER JOIN libros l ON l.id_libro = p.id_libro WHERE p.id_prestamo = ?');
    $consulta->execute([$id]);
    ...
    $conexion->prepare("UPDATE prestamos SET estado = 'DEVUELTO', fecha_devolucion_real = date('now', 'localtime') WHERE id_prestamo = ?")->execute([$id]);
    $conexion->prepare('UPDATE libros SET cantidad_total = MAX(cantidad_total, cantidad_disponible + 1), cantidad_disponible = cantidad_disponible + 1 WHERE id_libro = ?')->execute([$prestamo['id_libro']]);
    $conexion->commit();
